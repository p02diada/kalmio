...
- `VITE_KALMIO_MAP_STYLE_URL` (optional frontend MapLibre style URL; if omitted, route maps render Kalmio route/station layers on a local neutral background)
...
`MapPreviewCard` is a route inspection surface, not a frontend route planner. It renders MapLibre from validated `plan_route` geometry and traced station coordinates; when WebGL or a map style is unavailable, the renderer falls back to a static route drawing from the same coordinates. Charger discovery near a route remains a backend/agent action and must return through validated A2UI data.
