...
- `KALMIO_GEOCODING_PROVIDER` (default `mapbox`; development falls back to local known locations when no Mapbox token is configured)
- `KALMIO_MAPBOX_ACCESS_TOKEN` (required in production when `KALMIO_GEOCODING_PROVIDER=mapbox`)
