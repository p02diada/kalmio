...
class ResolveLocationArgs(ToolModel):
    ...
    searchMode: Literal["auto", "poi", "address", "place"] = Field(
        "auto",
        description="Pista opcional: poi, address, place o auto.",
    )
...
class SearchDestinationChargersArgs(ToolModel):
    ...
    purpose: Literal["urgent", "destination", "stay", "near_route_fallback"] = "destination"
    ...
    requested_services: list[str] = Field(default_factory=list, max_length=8)
    max_useful_power_kw: float | None = Field(None, ge=1, le=500)
    require_verified_price: bool = False
    min_total_evses: int | None = Field(None, ge=1, le=50)
    allow_unvalidated_safety: bool = True
    max_data_age_days: float | None = Field(None, ge=0)
...
class StationFact(ToolModel):
    ...
    effectivePowerKw: float | None = Field(None, ge=0)
    ...
    safetyValidated: bool | None = None
    accessNotes: str | None = None
    accessNotesVerified: bool | None = None
...
class SearchDestinationChargersResult(ToolModel):
    ...
    appliedFilters: dict[str, Any] | None = None
    filtersApplied: dict[str, Any] | None = None
