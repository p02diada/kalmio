...
class Command(BaseCommand):
    help = "Import vehicle profiles from an authorized Iternio/ABRP API key."
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--country-code3", help="Optional ISO 3166-1 alpha-3 country filter, e.g. ESP.")
        parser.add_argument(
            "--replace",
            action="store_true",
            help="Delete existing Iternio vehicle profiles before importing the fetched catalog.",
        )
        parser.add_argument(
            "--dry-run",
            action="store_true",
    ...
    def handle(self, *args, **options):
        ...
        if options["dry_run"]:
            ...
            options_count = result.options
            display_count = result.display_groups

        suffix = " No database changes were made." if options["dry_run"] else ""
        self.stdout.write(
            self.style.SUCCESS(
                f"{action} {vehicles} vehicle profiles, {options_count} option groups, "
                f"{display_count} display groups.{suffix}"
            )
        )
