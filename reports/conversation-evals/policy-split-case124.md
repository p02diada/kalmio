# Kalmio Conversation Eval Report

- Run: `policy-split-case124`
- Cases: `1`

## Assertions

| Evaluator | Passed | Total | Pass rate |
| --- | ---: | ---: | ---: |
| `case_acceptance` | `1` | `1` | `100.0%` |
| `expected_text_hint_present` | `1` | `1` | `100.0%` |
| `expected_tools_pass` | `1` | `1` | `100.0%` |
| `forbidden_tools_pass` | `1` | `1` | `100.0%` |
| `hard_contract_pass` | `1` | `1` | `100.0%` |
| `no_fabrication_failure` | `1` | `1` | `100.0%` |
| `no_fallback` | `1` | `1` | `100.0%` |
| `no_http_error` | `1` | `1` | `100.0%` |
| `no_llm_error` | `1` | `1` | `100.0%` |
| `no_tool_error` | `1` | `1` | `100.0%` |
| `no_tool_failure` | `1` | `1` | `100.0%` |
| `safety_pass` | `1` | `1` | `100.0%` |
| `task_success` | `1` | `1` | `100.0%` |
| `tool_policy_pass` | `1` | `1` | `100.0%` |
| `ui_family_pass` | `1` | `1` | `100.0%` |

## Score Averages

| Metric | Average |
| --- | ---: |
| `duration_ms` | `15768.27` |
| `estimated_cost_usd` | `0.004932` |
| `fallbacks` | `0.0` |
| `llm_calls` | `2.0` |
| `repairs` | `0.0` |
| `tool_calls` | `1.0` |

## Failed Task Success Cases

- Ninguno.
